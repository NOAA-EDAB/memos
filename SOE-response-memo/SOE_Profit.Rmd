...
The SSC asked for a refined indicator of net revenue to better proxy for profitability. In what follows I provide an overview of the analysis conducted, per SSC recommendations. Specifically, I present a net revenue assessment (gross revenue minus trip costs, for federally reporting trips), profitability ratio (gross revenue for all trips divided by average trip costs), and cointegration analysis of gross revenue and fuel prices, by ecological production unit between the years 2000 - 2023. The aim of this document is to provide information sufficient for the SSC to select a preferred indicator to present within the MAFMC's State of the Ecosystem Report and Ecosystem Approach to Fishery Management Risk Assessment. All dollar values are reported in $2023 real values.
...
```{r Profitability, echo=FALSE, fig.cap="Revenue, Cost, and Profitability Indices by region."}
...
Stationarity_results <- rbind(
  c("Missing Costs","Gross Revenue",Stationarity1@test.name,Stationarity1@lags,paste0(Stationarity1@teststat,p1, sep="")),
c("Missing Costs","Gross Revenue",Stationarity2@test.name,"15",paste0(Stationarity2@teststat,p2, sep="")))

Stationarity_results <- rbind(Stationarity_results,
c("Net Revenue Possible","Gross Revenue",Stationarity3@test.name,Stationarity3@lags,paste0(Stationarity3@teststat,p3, sep="")))

Stationarity_results <- rbind(Stationarity_results,
c("Net Revenue Possible","Gross Revenue",Stationarity4@test.name,"15",paste0(Stationarity4@teststat,p4, sep="")))

Stationarity_results <- rbind(Stationarity_results,
c("Missing Costs","Diesel Price",Stationarity5@test.name,Stationarity5@lags,paste0(Stationarity5@teststat,p5, sep="")))

Stationarity_results <- rbind(Stationarity_results,
c("Missing Costs","Diesel Price",Stationarity6@test.name,"15",paste0(Stationarity6@teststat,p6, sep="")))

Stationarity_results <- rbind(Stationarity_results,
c("Net Revenue Possible","Diesel Price",Stationarity7@test.name,Stationarity7@lags,paste0(Stationarity7@teststat,p7, sep="")))

Stationarity_results <- rbind(Stationarity_results,
c("Net Revenue Possible","Diesel Price",Stationarity8@test.name,"15",paste0(Stationarity8@teststat,p8, sep="")))
...
knitr::kable(Stationarity_results, format = "latex", linesep = "",
      col.names = c("Fleet Segment","Variable","Model","Lags","tau"),
      #align = aca,
      booktabs = T, 
      caption = "Unit root tests for Gross Revenue and Diesel prices.") %>%
  kable_styling(latex_options = "HOLD_position") %>%
  row_spec(0, bold = TRUE) %>% 
  kableExtra::add_footnote(" * * * _p_-value < 0.01,* * _p_-value < 0.05, * _p_-value < 0.1.",notation="symbol",threepart=TRUE) 
```

\elandscape

To further highlight the fact, table \@ref(tab:cointegration) presents a regression of gross revenue on diesel prices. The insignificant parameter estimate on diesel prices indicates that there is no possibility of cointegration between the two series.
