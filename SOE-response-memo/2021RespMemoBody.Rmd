...
We welcome suggestions for further analysis that would be most useful for the Councils to evaluate and potentially use these ecosystem overfishing indices. 
...
```{r pp-recon, fig.cap="Primary production reconstructed (dashed line) using the mean of satellite-derived values from 1998-2010 (points).", code = readLines("https://raw.githubusercontent.com/NOAA-EDAB/ecodata/master/chunk-scripts/human_dimensions_MAB.Rmd-pp-reconstructed.R")}
```

It is also interpreted in the context of the ecosystem overfishing indicators introduced this year, which suggest when ecosystem overfishing may have ocurred over the past 50 years. 
...
The MA SSC was interested in an expanded cold pool index. This year we introduced an index based on 
...
Both Councils were interested in more quantitative analysis linking environmental indicators, managed fish indicators, and fishery indicators to facilitate use of this information in management. Considerable progress has been made on linking environmental indicators to fish condition for multiple species, with an overview of preliminary results included in the SOE. The NE SSC commented that overall (total) biomass could be included in the analysis of fish condition; this has been included in the analysis, as well as local abundance and local biomass. The MA SSC commented that indices of growth (weight at age) used in stock assessments could also be included in the analysis. The fish condition working group examined these and other fish growth indices during 2020 and hopes to integrate this information in future analyses. Another component of the project evaluating potential links between fish condition and market prices is also ongoing.  
